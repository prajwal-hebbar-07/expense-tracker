  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <OverviewCard label="Total money in" value={formatAmount(overview.totalIncome)} tone="text-emerald-400" />
        <OverviewCard label="Total spent" value={formatAmount(overview.totalSpending)} tone="text-rose-400" />
      </div>

      <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
        <section className="surface overflow-hidden">
          <div className="border-b border-white/[0.06] px-5 py-4 sm:px-6">
            <h2 className="text-base font-semibold text-slate-100">Bank accounts</h2>
          ...
          {overview.accounts.length === 0 ? (
            <div className="grid min-h-64 place-items-center px-6 py-12 text-center">
              <div>
                ...
                <p className="mt-4 font-medium text-slate-200">Add your first bank account</p>
          ...
          {overview.unassignedCount > 0 && (
            <div className="border-t border-amber-400/10 bg-amber-400/[0.06] px-5 py-3 text-sm text-amber-300 sm:px-6">
        ...
        <form onSubmit={addAccount} className="surface p-5 sm:p-6 lg:sticky lg:top-8">
          <div className="flex items-center gap-3">
            <span className="grid h-10 w-10 place-items-center rounded-xl bg-emerald-400/10 text-emerald-400">
              <Plus className="h-5 w-5" />
            </span>
            <div>
              <h2 className="text-base font-semibold text-slate-100">Add account</h2>
...
function OverviewCard({ label, value, tone = 'text-slate-100' }: { label: string; value: string; tone?: string }) {
  return (
    <div className="surface flex items-center justify-between gap-4 p-5">
      ...
      <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-white/[0.05] text-slate-500">
  ...
  return (
    <div className="flex flex-wrap items-center gap-4 p-5 sm:flex-nowrap sm:p-6">
      ...
      <div className="ml-[60px] sm:ml-0 sm:text-right">
        ...
        <p className={`mt-1 font-semibold ${account.currentBalance >= 0 ? 'text-slate-100' : 'text-rose-400'}`}>
          {formatAmount(account.currentBalance)}
        </p>
      </div>
      <div className="ml-auto flex gap-1">
        <button type="button" onClick={() => setEditing(true)} className="rounded-lg p-2 text-slate-600 transition hover:bg-white/[0.06] hover:text-slate-300" aria-label={`Edit ${account.name}`}>
          <Pencil className="h-4 w-4" />
        </button>
        <button type="button" onClick={remove} disabled={busy} className="rounded-lg p-2 text-slate-600 transition hover:bg-rose-400/10 hover:text-rose-400 disabled:opacity-50" aria-label={`Remove ${account.name}`}>
